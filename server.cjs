...
// 4. Simular confirmação de pagamento (para testes/desenvolvimento)
app.post('/api/checkout/simulate-confirm', async (req, res) => {
    const { transactionId } = req.body;
    try {
        await pool.query(
            "UPDATE orders SET status = 'confirmed' WHERE transaction_id = $1 OR id::text = $1",
            [transactionId]
        );
        res.json({ success: true });
    } catch (err) {
